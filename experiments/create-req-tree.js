...
var foundations = Object.entries({
...
;[{
  ...
  var subprograms = Object.entries(program.subprograms)
  ;[].concat(...subprograms.map(e => e[1])).forEach(c => addCourse(reqs,c))
  
  subprograms.push(...foundations)

  var courseSubMap = subprograms.reduce((obj,[name,courses]) => {
    courses.forEach(n => {
      obj[n] = obj[n] || new Set()
      obj[n].add(name)
    })
  ...
  var usedPrograms = new Set()
  reqs.nodes().forEach(n => {
    reqs.node(n).programs = courseSubMap[n] ? Array.from(courseSubMap[n]) : [null]
    reqs.node(n).programs.filter(n => n).forEach(n => usedPrograms.add(n))
  })
  
  fs.writeFileSync(path.resolve(__dirname,`../backtothestart/programs/${program.name}.js`),`
window._reqTree = ${JSON.stringify(graphlib.json.write(reqs))};
window._subprograms = ${JSON.stringify(Array.from(usedPrograms))};
`);
