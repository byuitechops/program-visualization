...
function order(){
  const range = (s,e,p) => s+(e-s)*p
  // let console = {log(){}}
  var rows = Array.from(levels.loop()).map(i => levels.get(i))
  for(var diri = 0; diri < 20; diri++){
    let dir = !(diri%2)
    // display()
    for(let r = 0; r < rows.length; r++){
      var r0 = r > 0 ? rows[r-1] : []
      var r2 = r < rows.length-1 ? rows[r+1] : []
      var to = rows[r].map(w => ({w:w,np:g.predecessors(w).map(n => r0.indexOf(n)).concat(g.successors(w).map(n => r2.indexOf(n)))}))
      for(let i = 0; i < to.length; i++){
        let n = to[i]
        to[i].mean = n.np.reduce((a,b) => a+b,0)/n.np.length
        // console.log(n.np,i,to[i].mean)
      }
      to.sort((a,b) => a.mean-b.mean)
      Object.assign(rows[r],to.map(n => n.w))
      rows[r].length = to.length
    }
    // yield levels
  }
}
...
//           var neigh = [target+1,target,target-1]
//             .sort((a,b) => Math.abs(a-i)-Math.abs(b-i)) // Bias to where we used to be
